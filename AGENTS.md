...
- No desktop, o espaçamento interno e o ritmo vertical de `Sobre` para baixo devem manter respiro amplo sem condensar elementos: os parágrafos de `Sobre` usam `line-height: 1.65` e `gap: 48px` em relação à logo; os cards de `Serviços` mantêm `padding: 36px 32px` com `gap: 32px`; o card `Por que a Covil?` preserva `padding: 44px 48px`, `gap: 52px` entre colunas e `margin-top: 20px` entre parágrafos; a área de `Contato` mantém o card CTA com `padding: 24px 32px` e `margin-top: 36px` para o bloco de redes alternativas (`.contact-alt`); e o `Footer` utiliza `padding: 60px ... 40px` com `gap: 40px` entre colunas.
...
- Na home, a posição e calibração do hero em desktop (`HERO_CONTENT_OFFSET_Y = -100`, altura `calc(var(--stack-3-top-desktop) + 33.928571vw)`) e no mobile (`MOBILE_STACK_START = -50`, `height: 100svh`, logo com `padding-top: calc(74px + 60px)` no `.hero__identity-layer` e tagline com `padding-top: clamp(260px, 36vh, 290px)`) estão consolidadas e aprovadas. Não altere as alturas da hero, os offsets do conteúdo ou os pontos de início das stacks sem pedido explícito do usuário.
- A logo da seção `Sobre` (`.about-inner .about-logo`) permanece oculta apenas no modo mobile (`max-width: 760px`) via `display: none;`, mantendo a logo da hero intacta em todos os modos.
- Na feat `vagalume-motion` (ativa apenas no desktop em tema escuro), o vagalume da direita (`--5`) percorre o site conforme o scroll em uma trajetória em S (Saindo da direita na hero → sobre a logo na seção Sobre → varrendo os 3 serviços da esquerda para a direita → pela direita do card Por que a Covil → pousando sobre a bolinha verde do card do WhatsApp em Contato). A piscada (`firefly-blink`) permanece contínua e independente do scroll, com amortecimento suave por `lerp` e oscilações orgânicas. No mobile, em tema claro e em `prefers-reduced-motion`, a animação de scroll permanece desativada e preserva a posição original dos vagalumes da hero.
- Os cards de `Nossos Serviços` (`.service-card`) contam com a animação de borda spotlight interativa (gradiente radial guiado pelo cursor via `--card-pointer-x/y`), enquanto o card de `WhatsApp` (`.contact-card`) utiliza a elevação limpa. As animações de hover reagem de forma instantânea (transição de `0s` no hover para resposta imediata ao movimento do ponteiro) e o deslocamento vertical é mais próximo/curto (`translateY(-3px)`). Os ícones de `Nossos Serviços` mudam para a cor verde ao passar o mouse tanto no tema claro (`#057447`) quanto no escuro (`#55f289`).
- A página dedicada de contato (`/contato`) agrupa todos os canais e redes sociais oficiais da empresa (WhatsApp, Instagram, LinkedIn, E-mail e GitHub da organização Covil-Works), mantendo rigorosamente os padrões visuais, suporte aos temas claro/escuro e sem expor links pessoais da equipe.
- Na interação sutil do cursor com o cenário do Hero: no **Dark Mode**, os vagalumes (`.hero__firefly-inner` e `.traveling-firefly`) reagem ao cursor dentro de um raio de influência (~140px), sofrendo uma atração orgânica fraca (deslocamento suave de até ~32px via lerp); no **Light Mode**, os pássaros (`.hero__bird-inner`) realizam um desvio vertical sutil (até ~28px para cima ou para baixo dependendo da posição do mouse) sem interromper, alterar ou reiniciar a sua trajetória horizontal contínua (`bird-crossing`). As interações são suavizadas por interpolação em `requestAnimationFrame` e permanecem desativadas sob `prefers-reduced-motion` e em dispositivos de toque sem ponteiro fino.
...
- `MOBILE_STACK_START = -50` e `MOBILE_STACK_GAP = 10`.
