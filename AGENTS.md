...
- Na home, no desktop, o conteúdo do hero utiliza `HERO_CONTENT_OFFSET_Y = 0` para manter respiro adequado em relação ao cabeçalho. No mobile, a logo do hero é posicionada a pelo menos 60px do término do cabeçalho (com `padding-top: calc(74px + 60px)` no `.hero__identity-layer`), e as camadas do parallax utilizam `MOBILE_STACK_START = -80` para manter a silhueta das serras/stacks visível sem sumir no rodapé do hero.
- A logo da seção `Sobre` (`.about-inner .about-logo`) permanece oculta apenas no modo mobile (`max-width: 760px`) via `display: none;`, mantendo a logo da hero intacta.
