...
- Na home, no desktop, o conteúdo do hero preserva seu layout e constante `HERO_CONTENT_OFFSET_Y = -100` intactos. No modo mobile, a altura do hero é de `calc(100svh + 30px)`, a logo do hero fica a pelo menos 60px do término do cabeçalho (iniciando em `padding-top: calc(74px + 60px)` no `.hero__identity-layer`), o texto/tagline inicia em `padding-top: clamp(260px, 36vh, 290px)` eliminando qualquer sobreposição com a logo, e as camadas do parallax no mobile utilizam `MOBILE_STACK_START = -50` para manter as silhuetas visíveis.
